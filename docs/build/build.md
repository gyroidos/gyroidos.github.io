...
For more information on which files are needed, please refer to the [PKI section](../pki.md).
...
Build and include the debian/os installer image (see [example: Using GuestOs debos](../operate/examples.md#example-using-guestos-debos))
```
...
(see [example: Using docker-convertos](../operate/examples.md#example-using-docker-convertos))
```
...
In order to do so, please refer to the [Deploy section](../deploy/x86.md)

```
bitbake gyroidos-cml
```
## Build installer image
If required, a bootable installer image can be created. This image can be used to boot the target platform and install GyroidOS on the internal disk as described in the [Deploy section](../deploy/x86.md)
...
This image can be used to configure secure boot on your platform as described in section [Deploy](../deploy/x86.md).
```
