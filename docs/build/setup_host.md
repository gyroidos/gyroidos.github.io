...
2. Create and initialize workspace on host (for further information and available manifests see [build/initialize workspace](../build/build.md#initialize-workspace))
```
mkdir ~/ws-yocto
cd ~/ws-yocto
...
```
5. Follow build instruction from [Setup Yocto environment](../build/build.md#setup-yocto-environment) inside the Docker container
